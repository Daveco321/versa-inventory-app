...
function ImageWithFallback({ src, alt, style, className, onClick }) {
  ...
  useEffect(() => { setCurrentSrc(src); setError(false); triedRef.current.clear(); }, [src]);
  ...
  return <img src={currentSrc} alt={alt} style={style} className={className} onError={handleError} onClick={onClick} loading="lazy" />;
}
  ...
  return (
    <div onClick={onClick} className="product-card" style={{ background:"#fff",borderRadius:14,overflow:"hidden",border:"2px solid #e5e7eb" }}>
      <div style={{ position:"relative",overflow:"hidden" }}>
        <ImageWithFallback src={getImageUrl(item)} alt={item.sku} style={{ width:"100%",height:220,objectFit:"cover",background:"#f3f4f6" }} />
      </div>
      ...
      <div style={{ background:"rgba(255,255,255,.97)",borderRadius:16,maxWidth:960,width:"100%",maxHeight:"90vh",overflowY:"auto",boxShadow:"0 25px 60px rgba(0,0,0,.3)",padding:28 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:32 }}>
          ...
          <div style={{ textAlign:"center" }}>
            <ImageWithFallback src={getImageUrl(item)} alt={item.sku} style={{ maxWidth:"100%",maxHeight:400,borderRadius:12,objectFit:"contain",border:"2px solid #e5e7eb" }} />
            {ats > 0 && (
          ...
          <>
            {cart.map((c, i) => (
              <div key={i} style={{ display:"flex",alignItems:"center",gap:14,padding:14,border:"1px solid #e5e7eb",borderRadius:12,marginBottom:10 }}>
                <ImageWithFallback src={getImageUrl(c)} alt={c.sku} style={{ width:56,height:56,borderRadius:8,objectFit:"cover" }} />
                <div style={{ flex:1 }}>
        ...
        <div style={{ position:"absolute",top:"100%",left:0,right:0,background:"#fff",border:"2px solid #e5e7eb",borderTop:"none",borderRadius:"0 0 14px 14px",maxHeight:380,overflowY:"auto",zIndex:100,boxShadow:"0 10px 25px rgba(0,0,0,.12)" }}>
          {results.map((item, i) => (
            <div key={i} onClick={() => { onSelect(item); setOpen(false); setQuery(""); }}
              ...
              <ImageWithFallback src={getImageUrl(item)} alt={item.sku} style={{ width:48,height:48,objectFit:"cover",borderRadius:8,marginRight:14 }} />
              <div style={{ flex:1 }}>
...
export default function VersaInventoryApp() {
  ...
  const goToInventory = useCallback((brandKey) => { setCurrentBrand(brandKey); setView("inventory"); setSelectedItem(null); setSearchQuery(""); setFitFilter([]); setFabricFilter([]); }, []);
            ...
            <div style={{ background:"rgba(255,255,255,.04)",borderRadius:16,border:"1px solid rgba(255,255,255,.06)",padding:18,marginBottom:20 }}>
              ...
              {availableFits.length > 1 && (
                <div style={{ display:"flex",gap:6,flexWrap:"wrap",marginBottom:8 }}>
                  <span style={{ fontSize:11,fontWeight:700,color:"#64748b",lineHeight:"30px",marginRight:4 }}>FIT</span>
                  {availableFits.map(f => (
                    <button key={f} onClick={() => setFitFilter(prev => prev.includes(f) ? prev.filter(x => x!==f) : [...prev, f])}
                      className={`filter-pill ${fitFilter.includes(f) ? "active" : ""}`}>{f}</button>
                  ))}
                </div>
              )}
              ...
              {availableFabrics.length > 1 && (
                <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
                  <span style={{ fontSize:11,fontWeight:700,color:"#64748b",lineHeight:"30px",marginRight:4 }}>FABRIC</span>
                  {availableFabrics.slice(0,12).map(([code, desc]) => (
                    <button key={code} onClick={() => setFabricFilter(prev => prev.includes(code) ? prev.filter(x => x!==code) : [...prev, code])}
                      className={`filter-pill ${fabricFilter.includes(code) ? "active" : ""}`}>{code}: {desc.length > 14 ? desc.substring(0,12)+"…" : desc}</button>
                  ))}
                  {availableFabrics.length > 12 && <span style={{ fontSize:11,color:"#64748b",lineHeight:"30px" }}>+{availableFabrics.length-12} more</span>}
                </div>
              )}

              <p style={{ fontSize:12,color:"#64748b",marginTop:10 }}>
                Showing <strong style={{ color:"#e2e8f0" }}>{filteredItems.length}</strong> of {brandData.items.length} styles
                {(fitFilter.length > 0 || fabricFilter.length > 0 || searchQuery) && (
                  <button onClick={() => { setFitFilter([]); setFabricFilter([]); setSearchQuery(""); }}
                    style={{ marginLeft:8,fontSize:11,color:"#818cf8",background:"none",border:"none",cursor:"pointer",fontWeight:600 }}>Clear filters</button>
                )}
              </p>
            </div>
